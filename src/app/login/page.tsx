...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Check if user is already logged in via server-side verification
  ...
  const loadPageConfigs = async () => {
    try {
      const response = await fetch("/api/page-configs", { cache: "no-store" })
      const data = await response.json()
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 将用户信息写入 localStorage，与首页(page.tsx)的登录态判定保持一致，
      // 否则首页读不到 localStorage.user 会再次跳回 /login 形成死循环。
      if (data.user) {
      ...
      <div className="max-w-md w-full mx-4 animate-[fadeIn_0.5s_ease-in-out]">
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8 transform transition-all duration-300 hover:scale-[1.01]">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent mb-2 whitespace-pre-line">
              {String(pageConfigs.login_title || "济南黄河业余无线电台<br/>台网主控日志").replace(/<br\s*\/?>/gi, "\n")}
            </h1>
