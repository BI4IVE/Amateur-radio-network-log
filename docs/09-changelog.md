...
- 新增统一入口 formatDateTimeCN，覆盖后台用户/设备/排期/轮值/回收站/推送日志/绑定记录/配置更新时间与首页倒计时
...
- 主页与后台「新建台网」的默认时间同步改用服务器时间，避免主控所处时区不同或本机时钟不准导致入库时间偏差
...
- 修复深色环境下未显式设色的表单控件出现「白字白底」不可见的问题：移除全局深色媒体查询（全站本就是浅色设计，且未使用任何 dark: 工具类）
...
下版本将发布全新2.0版本将更新微信功能建议可等待2.0的发布。

## v1.5.20 (2026-09-01)

- 后台管理菜单重构为分组结构（运营中心 / 数据与日志 / 系统管理），层级更清晰、导航更高效。

## v1.5.19 (2026-08-31)

- 新增登录日志，记录登录 IP、时间、设备及成败原因（密码错误 / 账号不存在 / 账号锁定），便于识别异常登录与爆破。
- 新增数据看板（/admin/analytics），零依赖可视化呈现场次趋势、呼号活跃排行与按周分布。
- 新增大屏开放开关（screen_public），支持私有（仅登录可见）/ 公开（任何人可见）两种模式；公开模式 QTH 脱敏显示前 4 字，私有未登录显示美化遮罩。
- 修复台网预告创建相关问题：唯一索引冲突返回 409 友好提示（原笼统报 Failed query）；修复创建后列表因状态字段被剥离而不显示的问题。
