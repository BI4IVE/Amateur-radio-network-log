...
- 实况大屏接入真实数据：当前主控、最新参与者、台网人数、最近记录、信号报告（RST 解析）实时刷新
- 信号环根据信号报告（11~59 / 599）动态显示强度、颜色与可读性表述
- 底部显示今日日期，右上角发射/接收频率、亚音参数
- 修复：登录后旧缓存致页面白屏（前端自动刷新兜底 + 首页清理缓存按钮）
- 修复：删除当日台网后无法重新创建
- 修复：正式站多进程抢端口偶发加载旧版（统一 PM2 管理）
- 正式站静态资源缓存调整为 5 分钟
