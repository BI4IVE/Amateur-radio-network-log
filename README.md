...
- **登录日志**：记录登录 IP、时间、设备及成败原因（密码错误 / 账号不存在 / 账号锁定），便于识别异常登录与爆破。
- **数据看板**：`/admin/analytics` 零依赖可视化呈现场次趋势、呼号活跃排行与按周分布。
- **大屏开放开关**：后台可切换私有（仅登录可见）/ 公开（任何人可见）；公开模式 QTH 脱敏显示前 4 字，私有未登录显示美化遮罩。
- **台网预告修复**：唯一索引冲突返回友好提示；修复创建后列表不显示的问题。
